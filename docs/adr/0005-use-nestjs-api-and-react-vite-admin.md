...
OKVNS needs a REST API with validation, error mapping, health probes, and contract tests. It also needs a browser-based admin UI for namespace and entry management plus markdown import/export.
...
The API exposes REST endpoints for health, readiness, namespace CRUD, entry CRUD, markdown import, and markdown export. The admin frontend consumes those endpoints through an isolated API client layer instead of calling `fetch` directly from components.
