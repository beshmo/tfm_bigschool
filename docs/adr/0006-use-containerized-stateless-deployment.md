...
- Stateless replicas are straightforward; durable namespace/entry state now lives in MySQL (see [ADR-0008](0008-use-mysql-for-durable-storage.md)), not in API process memory.
- The API is stateless, but the platform now depends on a MySQL backing service with a persistent volume and Kubernetes Secrets for credentials. Queues and Redis remain out of scope.
