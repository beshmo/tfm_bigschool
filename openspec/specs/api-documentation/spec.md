...
The generated OpenAPI document SHALL describe API request bodies, response bodies, path parameters, validation constraints, multipart upload support, description fields, timestamp fields, and safe error shapes for documented endpoints.
...
- **THEN** it documents entry names, entry values, optional entry descriptions with the 1000-character limit, entry response bodies, entry `created_at` and `modified_at` fields, entry create and update request bodies, path parameters, success responses, and relevant error responses

#### Scenario: YAML import and export schemas are documented
- **WHEN** the OpenAPI document describes YAML endpoints
- **THEN** it documents JSON YAML import, multipart YAML file import, optional imported description fields, optional imported timestamp metadata, YAML export description and timestamp metadata, YAML export responses, success responses, and relevant validation or not-found error responses
