...
The admin frontend SHALL keep the structure below so the UI can be rebuilt from this spec. Visual tokens are owned by `apps/admin-web/src/styles.css` (the vendored Industry design system) and are vendored verbatim for reference in `docs/design-tokens.css` (tokens and `.blueprint` frame) and `docs/design-components.css` (component classes and app layout), both guarded against drift by a test, and explained in `docs/design-system.md`: every color, font, spacing, radius and shadow comes from a `--color-*`, `--font-*` (`--font-heading`, `--font-body`, `--font-heading-weight`), `--space-*` (1, 2, 3, 4, 6, 8), `--radius-*` (sm, md, lg) or `--shadow-*` (sm, md, lg) variable.
...
#### Scenario: Stylesheet matches the vendored sheets
- **WHEN** the admin app is built
- **THEN** `apps/admin-web/src/styles.css` is its own header comment followed by `docs/design-tokens.css` and then `docs/design-components.css` (their header comments excluded), which `apps/admin-web/src/design-tokens.test.ts` verifies
- **AND** framed elements use `.panel blueprint` with a `<Corners />` child, and no `.card` class exists
