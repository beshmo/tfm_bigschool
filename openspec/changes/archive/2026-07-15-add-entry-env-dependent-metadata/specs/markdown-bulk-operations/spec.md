...
- **THEN** the system stores all namespaces, namespace descriptions, entries, entry descriptions, and entry `env_dependent` values from the file
...
- **THEN** the system stores that namespace, its description, its entries, their descriptions, and their `env_dependent` values

#### Scenario: Import env_dependent defaults to false
- **WHEN** a client imports valid YAML containing an entry without `env_dependent`
- **THEN** the system stores that entry with `env_dependent` set to `false`
...
- **THEN** the system stores the namespaces, descriptions, entries, and entry `env_dependent` values from the uploaded file
...
#### Scenario: Invalid env_dependent in imported YAML is rejected
- **WHEN** imported YAML contains an entry `env_dependent` value that is not a boolean
- **THEN** the system returns an invalid YAML validation error without mutating storage
...
The system SHALL export namespaces and entries from durable storage as raw YAML using a canonical `namespaces` array shape. The exported YAML MUST NOT be wrapped in a markdown code fence, and MUST include namespace and entry `created_at` and `modified_at` metadata. The exported YAML MUST include namespace and entry `description` fields when descriptions are stored. The exported YAML MUST include entry `env_dependent` for every entry.
...
- **AND** each exported entry includes `env_dependent` as `true` or `false`
