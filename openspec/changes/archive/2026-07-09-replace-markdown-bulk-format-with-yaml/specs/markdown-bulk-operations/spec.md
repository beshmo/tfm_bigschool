...
**Reason**: The public bulk import contract is being replaced by raw YAML.
**Migration**: Clients MUST use `POST /yaml/import` with a `yaml` request field.
...
**Reason**: The public bulk import contract is being replaced by raw YAML.
**Migration**: Clients MUST submit YAML content and handle invalid YAML errors instead of invalid markdown errors.
...
**Reason**: The public bulk import contract is being replaced by raw YAML.
**Migration**: Clients MUST use YAML imports; atomic validation-before-mutation behavior is preserved.
...
**Reason**: The public bulk import contract is being replaced by raw YAML.
**Migration**: Clients MUST use YAML imports; namespace upsert behavior is preserved.
...
**Reason**: The public bulk export contract is being replaced by raw YAML.
**Migration**: Clients MUST use `GET /yaml/export` or `GET /yaml/export/:name` and read the `yaml` response field. Exported content is raw YAML, not fenced markdown.
