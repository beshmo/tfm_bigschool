...
The system SHALL allow API clients and admin users to create an entry with a valid unique name inside an existing namespace, storing the entry in durable storage and returning entry timestamp metadata.
...
The system SHALL allow API clients and admin users to list entries for a namespace, including entry timestamp metadata.
...
The system SHALL allow API clients and admin users to retrieve an entry by namespace name and entry name, including entry timestamp metadata.
...
The system SHALL allow API clients and admin users to update an entry name, value, or both within an existing namespace, committing the update to durable storage and updating entry modification metadata.
