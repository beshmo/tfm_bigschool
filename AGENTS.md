...
- Packages to create: `packages/domain`, `packages/application`, `packages/markdown`, `packages/shared`.
...
- Markdown import supports canonical `namespaces: [...]` and accepts the original single `namespace: ...` shape; export uses canonical `namespaces: [...]`.
- Markdown import validates the full document before mutating state, rejects unexpected keys, duplicate namespaces, duplicate entries, invalid names, non-string values, and oversized payloads.
...
- Use BDD-style test names when it improves clarity, e.g. `GIVEN valid markdown WHEN it is imported THEN namespaces are stored`.
